...
prepare_app() {
    ...
    for file in *; do
      ...
      envsubst '$INSTANCE_ID $PASSWORD_SALT $SECRET $TRUSTED_DOMAINS $DATA_DIRECTORY $DB_TYPE $DB_HOST $DB_NAME $DB_USER $DB_PASSWORD $DB_TABLE_PREFIX $DEFAULT_LANGUAGE $FORCE_LANGUAGE $DEFAULT_LOCALE $FORCE_LOCALE $KNOWLEDGE_BASE_ENABLED $ALLOW_USER_TO_CHANGE_DISPLAY_NAME $SKELETON_DIRECTORY $TEMPLATE_DIRECTORY $MAIL_DOMAIN $MAIL_FROM_ADDRESS $MAIL_SMTP_MODE $MAIL_SMTP_HOST $MAIL_SMTP_PORT $MAIL_SMTP_SECURE $MAIL_SMTP_AUTH $MAIL_SMTP_AUTH_TYPE $MAIL_SMTP_NAME $MAIL_SMTP_PASSWORD $OVERWRITE_HOST $OVERWRITE_PROTOCOL $OVERWRITE_WEBROOT $OVERWRITE_COND_ADDR $OVERWRITE_CLI_URL $MEMCACHE_LOCAL $VERSION $INSTALLED' \
